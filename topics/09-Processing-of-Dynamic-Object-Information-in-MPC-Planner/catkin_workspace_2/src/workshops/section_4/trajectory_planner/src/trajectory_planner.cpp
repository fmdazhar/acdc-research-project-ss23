...
SC CostTermIntermediate<STATE_DIM, CONTROL_DIM, MPC_NODE, SCALAR_EVAL, SCALAR>::evalLocal(const Eigen::Matrix<SC, STATE_DIM, 1> &x, const Eigen::Matrix<SC, CONTROL_DIM, 1> &u, const SC &t)
{
    ...
    // auto cg_value = CppAD::Value(t);
    // double val = cg_value.getValue();
    // t_values.push_back(val);
    ...
    return pathTerm + jerkLonTerm + jerkLatTerm + alphaTerm + velTerm + TrafficLightTerm;
}

// template <size_t STATE_DIM, size_t CONTROL_DIM, class MPC_NODE, typename SCALAR_EVAL, typename SCALAR>
// Eigen::VectorXd DistanceAvoidanceConstraint<STATE_DIM, CONTROL_DIM, MPC_NODE, SCALAR_EVAL, SCALAR>::evaluate(const state_vector_t &x, const control_vector_t &u, const SCALAR &t)
// {
...
//     size_t i = static_cast<size_t>(t / 0.2 ); // Assuming time step of 0.2

//     // Ensure i is within bounds of the vectors
//     if (i < x_pred_.size() && i < y_pred_.size()) {
//         SCALAR dist = (x[0] - x_pred_[i]) * (x[0] - x_pred_[i]) + (x[1] - y_pred_[i]) * (x[1] - y_pred_[i]);
//         val(0) = dist - d_safe_;
//     } else {
//         ROS_WARN_STREAM("Time index out of bounds: " << i);
//     }
...
void PLANNER::runMPC()
{
    ...
    // for(double time : t_values) {
        // std::cout << "time " << t_values[10] << std::endl;
