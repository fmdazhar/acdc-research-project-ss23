...
SC CostTermIntermediate<STATE_DIM, CONTROL_DIM, MPC_NODE, SCALAR_EVAL, SCALAR>::evalLocal(const Eigen::Matrix<SC, STATE_DIM, 1> &x, const Eigen::Matrix<SC, CONTROL_DIM, 1> &u, const SC &t)
{
    ...
    SC Length_dyn = x[MPC_NODE::DYNOBJCOORDS::len];
    SC dynObjRef = x[MPC_NODE::WEIGHTS::DYNOBJ_REF];

    // Old distance using coordinates only calculation
    ...
    // Addition of relative speed dependent term and capped (threshold) cost function
    // Calculate stopping distance
    // SC Width_ego = SC(1.855)
    // SC Length_ego = SC(4.711);
    // SC dynObjDist = CppAD::abs(CppAD::sqrt(CppAD::pow((dynObjX + (dynObjVx * t) + (0.5 * dynObjAx * t * t)) - x[0], 2) + CppAD::pow((dynObjY + (dynObjVy * t) + (0.5 * dynObjAy * t * t)) - x[1], 2))-((Length_ego+Length_dyn)/2));
    ...
    // SC a_max = SC(5.0);
    // SC stoppingDist = CppAD::pow(x[3], 2) / (2 * a_max);
    // SC k = SC(1.0);  // Adjust this parameter based on your requirements
    // SC sigmoidMod = 2 / (1 + CppAD::exp(k * (dynObjDist - 5*stoppingDist)));
    // // SC sigmoidMod = 1 / (1 + CppAD::exp(k * ((dynObjDist/stoppingDist)-5)));
    // SC dynObjCost = CppAD::CondExpLt(
    //     dynObjDist, 
    //     dynObjRef, 
    //     ( CppAD::cos(SC(M_PI) * CppAD::pow(dynObjDist, 2) / CppAD::pow(dynObjRef, 2)) + 1) *  (1 + sigmoidMod), 
    //     SC(0.0)    );
    // SC dynObjWeight = x[MPC_NODE::WEIGHTS::DYNOBJ];
    // SC dynObjTerm = CppAD::pow(dynObjCost * dynObjWeight, 2);
